...
const toBoliviaTime = (matchDate) => {
  ...
  try {
    ...
    const dia = dias[bol.getUTCDay()];
    return `${dia} ${bol.getUTCDate()}/${bol.getUTCMonth()+1} · ${String(bol.getUTCHours()).padStart(2,'0')}:${String(bol.getUTCMinutes()).padStart(2,'0')}`;
  } catch { return ''; }
};
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'4px 0', borderTop: isHome ? 'none' : '0.5px solid rgba(62,95,217,0.5)' }}>
      <div style={{ display:'flex', alignItems:'center', gap:5, overflow:'hidden' }}>
        ...
        <span style={{ fontSize:11, color: ph ? '#6B7FC7' : '#fff', fontStyle: ph ? 'italic' : 'normal', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', maxWidth:90 }}>
          {ph ? 'Por definir' : name}
        </span>
      </div>
...
function MatchCard({ match, prediction }) {
  if (!match) return (
    <div style={{ background:'#0E2272', borderRadius:6, padding:'5px 8px', borderLeft:'2px solid #3E5FD9', opacity:0.4, minWidth:138 }}>
  ...
  return (
    <div style={{ background:'#12277E', borderRadius:6, padding:'5px 8px', borderLeft:`2px solid ${borderColor}`, opacity: noneReal ? 0.55 : 1, minWidth:138, maxWidth:155 }}>
...
function BracketColumn({ ids, matches, predictions, flex=1 }) {
  return (
    <div style={{ display:'flex', flexDirection:'column', justifyContent:'space-around', flex, minWidth:155 }}>
      {ids.map(id => (
        <div key={id} style={{ display:'flex', alignItems:'center', flex:1, padding:'4px' }}>
  ...
  return (
    <div style={{ padding:'16px 12px', maxWidth:1400, margin:'0 auto' }}>
      <div style={{ marginBottom:16 }}>
        ...
        <p style={{ fontSize:12, color:'#7C90D9', margin:'3px 0 0' }}>Los equipos se colocan solos a medida que se definen los cruces</p>
      </div>
      <div style={{ overflowX:'auto', WebkitOverflowScrolling:'touch' }}>
        <div style={{ display:'flex', minWidth:1100, minHeight:700, background:'#0A1F66', borderRadius:12, border:'1px solid rgba(255,184,0,0.2)', padding:12 }}>
          <BracketColumn ids={BRACKET_LEFT.r32} matches={matches} predictions={predictions} flex={2} />
          <BracketColumn ids={BRACKET_LEFT.r16} matches={matches} predictions={predictions} />
          <BracketColumn ids={BRACKET_LEFT.qf} matches={matches} predictions={predictions} />
          <BracketColumn ids={BRACKET_LEFT.sf} matches={matches} predictions={predictions} />
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minWidth:165, padding:'0 8px', gap:16 }}>
            <div style={{ textAlign:'center' }}>
              <div style={{ fontSize:22, marginBottom:4 }}>🏆</div>
              <div style={{ fontSize:13, fontWeight:500, color:'#FFB800', letterSpacing:1 }}>FINAL</div>
              <div style={{ marginTop:8 }}><MatchCard match={matches['FINAL']} prediction={predictions['FINAL']} /></div>
            </div>
          ...
          <BracketColumn ids={[...BRACKET_RIGHT.sf].reverse()} matches={matches} predictions={predictions} />
          <BracketColumn ids={[...BRACKET_RIGHT.qf].reverse()} matches={matches} predictions={predictions} />
          <BracketColumn ids={[...BRACKET_RIGHT.r16].reverse()} matches={matches} predictions={predictions} />
          <BracketColumn ids={[...BRACKET_RIGHT.r32].reverse()} matches={matches} predictions={predictions} flex={2} />
        </div>
      </div>
